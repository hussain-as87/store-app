        ...
        <form action="{{route('about.update')}}" method="post" enctype="multipart/form-data">
            ...
            <div class="tab-content" id="myTabContent">
                ...
                @foreach(config('locales.languages') as $key => $val)
                    <div id="home{{$key}}" class="container tab-pane {{ $loop->index == 0 ? 'show active' : '' }}"
                        ...
                        <div class="form-group">
                            ...
                            <textarea placeholder="@error('story.' . $key){{ $message }} @enderror"
                                      name="story[{{ $key }}]" class="form-control" rows="10">
                 {!! old('story.' . $key, $about->getTranslation('story', $key)) ?? $about->story !!}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="story">{{__('service')}}({{ __($val['name']) }})</label>
                            <textarea placeholder="@error('service.' . $key){{ $message }} @enderror"
                                      name="service[{{ $key }}]" class="form-control" rows="8">
                ...
                <div class="form-group">
                    ...
                    <input id="story_image" class="form-control" type="file" name="story_image"
                           value={{ old('story_image') }}>
                </div>

                <div class="form-group">
                    <label for="service_image">{{__('service image')}}</label>
                    <input id="service_image" class="form-control" type="file" name="service_image"
                           value={{ old('service_image') }}>
                </div>
            </div>
            <a class="btn badge-warning" href="{{route('about.index')}}">{{__('Go Back')}} <i
