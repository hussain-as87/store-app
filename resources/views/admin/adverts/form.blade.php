...
<div class="tab-content" id="myTabContent">
    @if ($advert->image)
        <div class="center">
            <img src="{{ asset('storage/advertis/' . $advert->image) }}"
        ...
        <div id="home{{ $key }}" class="container tab-pane {{ $loop->index == 0 ? 'show active' : '' }}"
            ...
            <div class="form-group">
                ...
                <input type="text" name="title[{{ $key }}]"
                       class="form-control @error('title.' . $key) is-invalid alert-danger @enderror" id="title"
                       value="{!! old('title.' . $key, $advert->getTranslation('title', $key)) ?? $advert->title !!}"
                       aria-describedby="emailHelp" placeholder="@error('title.' . $key){{ $message }} @enderror">
            </div>
