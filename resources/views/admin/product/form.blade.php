    ...
    @if ($product->image)
        <div class="center">
            <img src="{{ asset('storage/products/' . $product->image) }}"
                 style="width: 200px;position: relative;right: -420px;">
        </div>
    @endif
        ...
        <div id="home{{ $key }}" class="container tab-pane {{ $loop->index == 0 ? 'show active' : '' }}"
            ...
            <div class="form-group">
                ...
                <textarea placeholder="@error('description.' . $key){{ $message }} @enderror"
                          name="description[{{ $key }}]" class="form-control" id="description[{{ $key }}]" rows="5">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <select name="size[]" id="size" class="form-control" multiple>
                    <option value="S" {{  str_contains($product->size,  'S') ? 'selected' : '' }}>{{ __('size') }}S
                    </option>
                    <option value="M" {{ str_contains($product->size, 'M') ? 'selected' : '' }}>{{ __('size') }}M
                    </option>
                    <option value="L" {{ str_contains($product->size, 'L') ? 'selected' : '' }}>{{ __('size') }}L
                    </option>
                    <option value="XL" {{str_contains( $product->size,  'XL') ? 'selected' : '' }}>{{ __('size') }}XL
                    </option>
                    <option value="XXL" {{ str_contains($product->size , 'XXL') ? 'selected' : '' }}>{{ __('size') }}
                        XXL
                    </option>
                    <option value="XXXL" {{ str_contains($product->size,  'XXXL') ? 'selected' : '' }}>{{ __('size') }}
                        XXXL
                    </option>
                </select>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <select name="color[]" id="color" class="form-control" multiple>
                        <option
                            value="red" {{ str_contains($product->color ,  'red')  ? 'selected' : '' }}>{{__('Red')}} </option>
                        <option value="yellow" {{  'yellow' ? 'selected' : '' }}>{{__('Yellow')}} </option>
                        <option
                            value="blue" {{  str_contains($product->color ,  'blue')  ? 'selected' : '' }}>{{__('Blue')}} </option>
                        <option
                            value="white" {{  str_contains($product->color ,  'white')  ? 'selected' : '' }}>{{__('White')}} </option>
                        <option
                            value="grey" {{  str_contains($product->color ,  'grey')  ? 'selected' : '' }}>{{__('Grey')}} </option>
                        <option
                            value="green" {{  str_contains($product->color ,  'green')  ? 'selected' : '' }}>{{__('Green')}} </option>
                        <option
                            value="black" {{  str_contains($product->color ,  'black')  ? 'selected' : '' }}>{{__('Black')}} </option>
                    </select>
                </div>
            </div>
        </div>
